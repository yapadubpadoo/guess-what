...
def tokenize(text):
    r = requests.post('http://35.164.146.20:3031/tokenize-text', data = {'text':text})
    return r.json()['data']
    ...
    if data['field'] == 'feed' \
        ...
        if data['value']['parent_id'] == data['value']['post_id']:
            ...
            try:
                ...
                ticket['created_time'] = arrow.get(ticket['created_time']).to('Asia/Bangkok').format('YYYY-MM-DD hh:ii:ss')
